...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "vseledkin")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "vseledkin" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "vseledkin")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "vseledkin")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "vseledkin")
	tr.AddRouter("/shop/:sd/account", "vseledkin")
	t3 := NewTree()
