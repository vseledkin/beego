...
import (
	...
	"github.com/vseledkin/beego/context"
	"github.com/vseledkin/beego/logs"
)
...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "vseledkin") != "/v1/vseledkin/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/vseledkin/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/vseledkin", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "vseledkin" {
		t.Errorf("post func should vseledkin")
	}
}
